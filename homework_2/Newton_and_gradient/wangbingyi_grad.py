...
def get_func(x, y, f_name='f'):
    ...
    if f_name == 'f':                   #因为x y 有时候是meshgrid，所以运行到res['hessian']会出Warning
        return res[f_name]              #一定程度上也能减少计算量
...
class GRAD(object):
    def __init__(self):
        ...
        self.alpha = 0.01          #学习率一样的东西
        self.epsilon = 0.01        #递归终止条件
        ...
        self.need_init_plt = True        #是否需要初始化plt，就是等高线图
        self.ax = Axes3D(plt.figure(3), auto_add_to_figure=False)
        plt.figure(3).add_axes(self.ax)  # matplotlab要求这么做的，否则就给warning
    ...
    def one_iter(self):
        ...
        self.X = self.X - self.grad*self.alpha    # x减梯度乘以学习率
    ...
    def add_one_point(self, x=3, y=3, is_random=False):
        ...
        while self.judje() == 0:  #judje返回0代表还没收敛，返回1代表收敛
            ...
            cnt += 1
            if cnt > 1000 or np.max(np.abs(self.X)) > 7:    #超过1000次迭代梯度还比较大，则认为是不收敛
                self.draw_line()
                self.analyse = '不收敛'
                print('不收敛')
                break
    ...
    def draw_line(self):
        ...
        if self.need_init_plt:    #初始化第一张图的等高线图，热力图
            ...
            zz = get_func(xx, yy, 'f')
            plt.figure(1)
            plt.title("Grad", fontsize=20)    # 图像标题
        ...
        plt.xlabel('iterator count', fontsize=14)
...
class OPGUI(object):
    ...
    def clr_plt(self):
        ...
        plt.savefig(self.save_name3)
        self.show_plt()
        self.grad_ins.need_init_plt = True
    ...
    def add_one(self, is_rand):
        ...
        :return:
        """
        self.grad_ins.alpha = float(self.input_lr.get())
        if is_rand:
            self.grad_ins.add_one_point(is_random=True)
        else:
            self.grad_ins.add_one_point(float(self.inputx.get()), float(self.inputy.get()))
        self.label_log.config(text=self.grad_ins.analyse)
        self.show_plt()
...
if __name__ == '__main__':
    ...
    grad_ins = GRAD()
    for i in range(10):
        grad_ins.add_one_point(is_random=True)
    plt.show()
